...
export async function LangAppTrigger(request: HttpRequest, context: InvocationContext): Promise<HttpResponseInit> {
    ...
    try {
        ...
        if (page < 1 || limit < 1) {
            return {
                status: 400,
                body: JSON.stringify({ error: "Page and limit must be positive numbers" }),
...
export async function ReturnSizeOfData(request: HttpRequest, context: InvocationContext): Promise<HttpResponseInit> {
    context.log(`Http function processed request for url "${request.url}"`);
